// scripts/scraper-multiciudad.js
...
async function guardarPropiedad(prop, page, ciudad) {
  ...
  if (imagenesUrls.length > 0) {
    ...
    if (publicUrls.length > 0) {
      ...
      console.log(`  ✅ Guardada con ${publicUrls.length} fotos: ${prop.titulo}`);
    }
  } else {
...
async function scrapearLocalidad(page, localidad) {
  console.log(`\n📍 Scrapeando Argenprop — ${localidad.nombre}...`);
  const propiedades = [];
  const maxPaginas = 3;

  for (let paginaActual = 1; paginaActual <= maxPaginas; paginaActual++) {
    const url = paginaActual === 1
      ? `https://www.argenprop.com/inmuebles/venta/${localidad.slug}`
      : `https://www.argenprop.com/inmuebles/venta/${localidad.slug}-pagina-${paginaActual}`;
    ...
    try {
      ...
      if (items.length === 0) { console.log(`  Sin resultados, pasando.`); break; }
      ...
      console.log(`  ⚠️ Error en página ${paginaActual}: ${e.message}`);
      break;
    }
  }

  console.log(`  Total ${localidad.nombre}: ${propiedades.length} propiedades`);
  for (const prop of propiedades) {
    await guardarPropiedad(prop, page, localidad.nombre);
  }
}

async function main() {
  console.log('🚀 Iniciando scraper multi-ciudad Urbix...');
  ...
  try {
    for (const localidad of LOCALIDADES) {
      await scrapearLocalidad(page, localidad);
      await page.waitForTimeout(3000);
    }
    console.log('\n✅ Scraping multi-ciudad completado');
  } catch (e) {
