...
function PropCard({ p, operacion }: { p: Propiedad; operacion: Operacion }) {
  ...
  return (
    <Link href={`/propiedad/${p.id}`} className="group block bg-white rounded-2xl overflow-hidden border border-zinc-100 hover:shadow-xl hover:-translate-y-0.5 transition-all duration-300">
      ...
      <div className="p-4">
        ...
        <p className="text-xs text-zinc-400 line-clamp-1 mb-2">{p.direccion}</p>
...
export default function LocalidadClient({ ciudadNombre }: { ciudadNombre: string }) {
  ...
  const filtradas = todas.filter(p => p.operacion === operacion)
    ...
    <div className="min-h-screen bg-zinc-50 font-sans">
      ...
      <div className="max-w-7xl mx-auto px-4 py-8">
        ...
        <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4 mb-8">
          ...
          <div className="flex items-center bg-white border border-zinc-200 rounded-full p-1 shadow-sm self-start md:self-auto">
            <button
              onClick={() => cambiarOperacion('venta')}
            ...
            <button
              onClick={() => cambiarOperacion('alquiler')}
        ...
        {loading ? (
          ...
          <div className="text-center py-24 text-zinc-400">
            <p className="text-5xl mb-4">{operacion === 'venta' ? '🏠' : '🔑'}</p>
            <p className="font-semibold text-lg text-zinc-500">No hay propiedades en {operacion} en {ciudadNombre}</p>
            <p className="text-sm mt-2">{operacion === 'venta' ? 'Probá viendo propiedades en alquiler' : 'Probá viendo propiedades en venta'}</p>
            <button
              onClick={() => cambiarOperacion(operacion === 'venta' ? 'alquiler' : 'venta')}
              className="mt-6 bg-rose-500 hover:bg-rose-600 text-white font-semibold px-6 py-2.5 rounded-full transition text-sm"
            >
              Ver en {operacion === 'venta' ? 'alquiler' : 'venta'}
            </button>
          </div>
        ) : (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
