...
export default async function PropiedadDetalle({ params }: { params: Promise<{ id: string }> }) {
  ...
  const imgs: string[] = Array.isArray(p.imagenes) ? p.imagenes : []
    ...
    <div className="min-h-screen bg-zinc-50">
      ...
      <nav className="bg-white border-b border-zinc-200 sticky top-0 z-10">
        <div className="max-w-6xl mx-auto px-4 py-3 flex items-center gap-2 text-sm">
          <Link href="/" className="text-rose-500 font-bold text-lg">urbix</Link>
          <span className="text-zinc-300">/</span>
          <Link href="/" className="text-zinc-400 hover:text-zinc-600">Chivilcoy</Link>
          <span className="text-zinc-300">/</span>
          <span className="text-zinc-500 truncate max-w-sm">{p.titulo}</span>
        </div>
      </nav>
      ...
      <div className="max-w-6xl mx-auto px-4 py-8">
        ...
        {/* TÍTULO + DIRECCIÓN */}
        <div className="mb-4">
          <h1 className="text-2xl font-bold text-zinc-900 leading-tight mb-1">{p.titulo}</h1>
        ...
        {/* GALERÍA */}
        <div className="mb-6 rounded-2xl overflow-hidden bg-zinc-200 h-80 flex items-center justify-center relative">
          {imgs.length > 0 ? (
            <div className="grid grid-cols-4 grid-rows-2 gap-1 w-full h-full">
              <div className="col-span-2 row-span-2 overflow-hidden">
                <img src={imgs[0]} alt={p.titulo} className="w-full h-full object-cover hover:scale-105 transition duration-300" />
              </div>
              {[1,2,3,4].map(i => (
                <div key={i} className="overflow-hidden relative">
                  {imgs[i] ? (
                    <img src={imgs[i]} alt="" className="w-full h-full object-cover hover:scale-105 transition duration-300" />
                  ) : (
        ...
        {/* PRECIO + CTA */}
        <div className="bg-white rounded-2xl shadow-sm border border-zinc-100 px-6 py-5 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
          <div>
            <p className="text-3xl font-bold text-rose-500">{p.moneda} {p.precio?.toLocaleString("es-AR")}</p>
            {precioM2 && <p className="text-sm text-zinc-400 mt-1">{p.moneda} {precioM2.toLocaleString("es-AR")}/m²</p>}
          </div>
          <div className="flex gap-3">
            {p.url_origen && (
              <a href={p.url_origen} target="_blank" rel="noopener noreferrer"
                className="bg-rose-500 hover:bg-rose-600 text-white font-semibold px-6 py-3 rounded-xl transition text-sm">
                Ver en {p.fuente} →
              </a>
            )}
          </div>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-6">
          {[
            ...
            <div key={i} className="bg-white rounded-xl border border-zinc-100 shadow-sm p-4 flex items-center gap-3">
              <div className="w-9 h-9 bg-rose-50 rounded-lg flex items-center justify-center shrink-0">
                <svg className="w-5 h-5 text-rose-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">{item.svg}</svg>
              </div>
              <div>
                <p className="text-xs text-zinc-400">{item.label}</p>
                <p className="text-base font-bold text-zinc-800">{item.value}</p>
              </div>
            </div>
          ))}
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="lg:col-span-2 bg-white rounded-2xl border border-zinc-100 shadow-sm p-6">
            ...
            <p className="text-zinc-600 text-sm leading-relaxed whitespace-pre-line">{p.descripcion}</p>
          </div>

          {/* CONTACTO */}
          <div className="bg-white rounded-2xl border border-zinc-100 shadow-sm p-6 h-fit sticky top-20">
            <h2 className="text-sm font-semibold text-zinc-400 uppercase tracking-wider mb-1">Publicado por</h2>
            <p className="text-rose-500 font-bold text-base mb-1">{p.inmobiliaria || p.fuente}</p>
            {p.contacto && <p className="text-sm text-zinc-500 mb-4">{p.contacto}</p>}
            {p.url_origen && (
              <a href={p.url_origen} target="_blank" rel="noopener noreferrer"
                className="block w-full text-center bg-rose-500 hover:bg-rose-600 text-white font-semibold py-3 rounded-xl transition text-sm mb-3">
                Ver publicación completa
              </a>
            )}
            <div className="bg-zinc-50 rounded-xl p-3">
              <p className="text-xs text-zinc-400 text-center leading-relaxed">
                Urbix muestra propiedades de portales como {p.fuente}. La información puede variar. Verificá siempre con la inmobiliaria.
              </p>
            </div>
          </div>
